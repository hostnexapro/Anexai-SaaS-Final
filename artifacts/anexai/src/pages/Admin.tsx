...
export default function Admin() {
  ...
  // Load existing placeholder/masked data when fetched
  useEffect(() => {
    if (settings) {
  ...
  if (!isAdmin) {
    return (
      <div className="h-[60vh] flex items-center justify-center">
        <div className="glass-panel p-12 text-center rounded-3xl max-w-md relative overflow-hidden">
          ...
          <p className="text-muted-foreground">
            You need administrator privileges to access this area. Your current role is restricted.
          </p>
        </div>
      </div>
    );
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      // Only send values if they were changed (not the mask bullets)
      const payload: any = {};
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      <div className="glass-panel rounded-3xl p-1 relative overflow-hidden">
        {/* Decorative background element inside panel */}
